...
class Bot(object):
    ...
    def act(self, xdif, ydif,x1dif ,y1dif):
        """
        Chooses the best action with respect to the current state - Chooses 0 (don't flap) to tie-break
        """
        state = self.map_state(xdif, ydif, x1dif, y1dif)
    ...
    def map_state(self, xdif, ydif, x1dif,y1dif):
        """
        Map the (xdif, ydif, vel) to the respective state, with regards to the grids
        The state is a string, "xdif_ydif_vel"

        X -> [-40,-30...180] U [140, 210 ... 420]
        ...
        if x1dif < 480:
            x1dif = int(x1dif) - (int(x1dif)%15)
        else:
            x1dif = int(x1dif) - (int(x1dif)%100)
        if y1dif < 180:
            y1dif = int(y1dif) - (int(y1dif)%10)
        else:
            y1dif = int(y1dif) - (int(y1dif)%70)
        return str(int(xdif))+'_'+str(int(ydif))+'_'+str(int(x1dif))+"_"+str(int(y1dif))
