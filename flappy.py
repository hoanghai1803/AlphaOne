...
def mainGame(movementInfo):
    ...
    while True:
        if -playerx + lowerPipes[0]["x"] >-30:
            mypipe =lowerPipes[0]
            mypipe1 =upperPipes[0]
        else:
            mypipe = lowerPipes[1]
            mypipe1 =upperPipes[1]
        
        a = sqrt( (playerx - mypipe["x"])**2+ (playery -mypipe["y"])**2)
        b = sqrt((playerx - mypipe1["x"])**2+ (playery -mypipe1["y"])**2)

        if -playerx + mypipe["x"] <0:
            a =-a
            b =-b
        for event in pygame.event.get():
            if event.type == QUIT or (event.type == KEYDOWN and event.key == K_ESCAPE):
